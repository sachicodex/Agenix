...
class _CalendarDayViewScreenState extends ConsumerState<CalendarDayViewScreen>
  ...
  static const double _defaultHourHeight =
      TimelineZoomController.defaultHourHeight;
  ...
  late final TimelineZoomController _timelineZoom;
  double get _hourHeight => _timelineZoom.hourHeight.value;
  ...
  bool _isTouchPinchZoomActive = false;
  ...
  void _handleSyncStatusChanged(AsyncValue<SyncStatus> syncState) {
    ...
    if (_isUserTriggeredSyncActive && nextSyncing) {
      ...
      _isUserTriggeredSyncActive = false;
      _syncRotationController
        ..stop()
        ..reset();
    }
  ...
  Future<void> _runUserTriggeredSync() async {
    if (!_isUserTriggeredSyncActive) {
      setState(() {
        _isUserTriggeredSyncActive = true;
      });
    }
    if (!_syncRotationController.isAnimating) {
      _syncRotationController.repeat();
  ...
  double _timeColumnWidthFor(BuildContext context) {
    final base = _isMobileLayout(context) ? 56.0 : 68.0;
    if (_hourHeight >= 150) return base + 4;
    return base;
  }
  ...
  void _subscribeToEvents() {
    ...
    _eventsSubscription = _repository.watchEvents(_currentRange).listen((
      ...
      final nextEventsMap = <String, CalendarEvent>{
        for (final event in resolvedEvents) event.id: event,
      };
      final nextPartition = _partitionEvents(nextEventsMap.values);
      ...
      void applyStreamUpdate() {
        if (!mounted) return;
        final changed =
            !_sameEventMap(_eventsMap, nextEventsMap) ||
            !_sameEventList(_allDayEvents, nextPartition.allDay) ||
            !_sameEventList(_timedEvents, nextPartition.timed);
        ...
        setState(() {
          _eventsMap
            ..clear()
            ..addAll(nextEventsMap);
          _allDayEvents = nextPartition.allDay;
          _timedEvents = nextPartition.timed;
          _isLoading = false;
          _hasLoadedEventsOnce = true;
        });
      }
  ...
  void _scrollToNowOnInitialOpen() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final nowPosition = ((now.hour * 60 + now.minute) / 60) * _hourHeight;
  ...
  ScrollPhysics _timelineScrollPhysics() {
    if (_isTouchPinchZoomActive ||
        _isDraggingToCreate ||
        _isDraggingEvent ||
        _resizingEventId != null) {
      return const NeverScrollableScrollPhysics();
    }
    return const TimelineZoomScrollPhysics();
  }

  Widget _buildTimelineScrollArea({
    ...
    return TimelineZoomViewport(
      scrollController: _dayGridScrollController,
      zoomController: _timelineZoom,
      hourHeightListenable: _timelineZoom.hourHeight,
      physics: physics,
      scrollBehavior: scrollBehavior,
      timelineBuilder: timelineBuilder,
      onTouchPinchZoomStateChanged: _handleTouchPinchZoomStateChanged,
    );
  }

  void _handleSwipePointerDown(PointerDownEvent event) {
    if (_isTouchPinchZoomActive) return;
  ...
  void _handleTouchPinchZoomStateChanged(bool active) {
    if (_isTouchPinchZoomActive == active) return;
    if (active) {
      _resetSwipeTracking();
      _recoverFromStaleMobileInteractionState();
    }
    setState(() {
      _isTouchPinchZoomActive = active;
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && _isRouteVisible) {
      ...
      unawaited(
        _syncService.incrementalSync().catchError((Object error) {
          debugPrint('Resume pull sync failed: $error');
          return false;
        }),
      );
      unawaited(
        _syncService.pushLocalChanges().catchError((Object error) {
          debugPrint('Resume push sync failed: $error');
        }),
      );
      _refreshUserInfo();
    }
  ...
  ({List<CalendarEvent> allDay, List<CalendarEvent> timed}) _partitionEvents(
    Iterable<CalendarEvent> events,
  ) {
    final allDay = <CalendarEvent>[];
    final timed = <CalendarEvent>[];
    for (final event in events) {
      if (_isAllDayForCurrentDay(event)) {
        allDay.add(event);
      } else {
        timed.add(event);
      }
    }
    allDay.sort((a, b) => a.startDateTime.compareTo(b.startDateTime));
    timed.sort((a, b) => a.startDateTime.compareTo(b.startDateTime));
    return (allDay: allDay, timed: timed);
  }

  void _updateEventLists() {
    final partition = _partitionEvents(_eventsMap.values);
    _allDayEvents = partition.allDay;
    _timedEvents = partition.timed;
  }

  bool _sameEventMap(
    Map<String, CalendarEvent> current,
    Map<String, CalendarEvent> next,
  ) {
    if (identical(current, next)) return true;
    if (current.length != next.length) return false;
    for (final entry in current.entries) {
      final other = next[entry.key];
      if (other == null || !_sameEvent(entry.value, other)) {
        return false;
      }
    }
    return true;
  ...
  bool _sameEventList(List<CalendarEvent> current, List<CalendarEvent> next) {
    if (identical(current, next)) return true;
    if (current.length != next.length) return false;
    for (var i = 0; i < current.length; i++) {
      if (!_sameEvent(current[i], next[i])) {
        return false;
      }
    }
    return true;
  ...
  bool _sameEvent(CalendarEvent a, CalendarEvent b) {
    ...
    if (a.updatedAtRemote != b.updatedAtRemote) return false;
  ...
  Widget _buildDayView() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final timelinePhysics = _timelineScrollPhysics();
  ...
  List<Widget> _buildMultiDayEventWidgets(
    ...
    for (final event in _timedEvents) {
      for (var dayIndex = 0; dayIndex < days.length; dayIndex++) {
        ...
        final height = (((endMinutes - startMinutes) / 60) * _hourHeight).clamp(
          1.0,
          24 * _hourHeight,
        );
  ...
  Widget _buildReadOnlyEventCard(
    CalendarEvent event, {
  ...
  void _scrollToTime(DateTime dateTime) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final pos = ((dateTime.hour * 60 + dateTime.minute) / 60) * _hourHeight;
      final viewport = _dayGridScrollController.position.viewportDimension;
  ...
  Widget _buildCurrentTimeIndicator(BoxConstraints constraints) {
    ...
    final position = ((hour * 60 + minute) / 60) * _hourHeight;
  ...
  Widget _buildEventWidget(
    ...
    final startPosition = (startMinutes / 60) * _hourHeight;
    ...
    final maxTop = (24 * _hourHeight) - interactionHeight;
  ...
  DateTime _timeFromPosition(Offset localPosition) {
    final y = localPosition.dy.clamp(0.0, 24 * _hourHeight);
    final minutes = (y / _hourHeight) * 60;
