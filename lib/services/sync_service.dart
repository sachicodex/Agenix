...
class SyncService {
  ...
  Future<void> pushLocalChanges({bool retryWhenLocked = false}) async {
    ...
    if (!await _tryAcquireSyncLock()) {
      ...
      if (retryWhenLocked) {
        ...
        if (await _tryAcquireSyncLock()) {
          try {
            final shouldPublishStatus = _status.state != SyncState.syncing;
            if (shouldPublishStatus) {
              _setStatus(const SyncStatus(state: SyncState.syncing));
            }
            try {
              await _runQueuedPushLocalChanges();
            } finally {
              if (shouldPublishStatus) {
                _setStatus(
                  SyncStatus(
    ...
    try {
      await inFlight;
      DebugPerfLogger.end('SyncService', watch, 'pushLocalChanges');
    } finally {
  ...
  void _scheduleDeferredPush() {
    ...
    DebugPerfLogger.info('SyncService', 'scheduleDeferredPush');
  ...
  Future<void> _pushLocalChangesOnce() async {
    final batchWatch = DebugPerfLogger.start(
      'SyncService',
      'pushLocalChangesOnce',
    );
    ...
    for (final event in pending) {
      final eventWatch = DebugPerfLogger.start('SyncService', 'pushEvent');
      final latest = await _localStore.getById(event.id);
      if (latest == null) {
        DebugPerfLogger.end(
          'SyncService',
          eventWatch,
          'pushEvent.skipMissing',
          data: {'id': event.id},
        );
        continue;
      }
      ...
      try {
        if (normalizedEvent.pendingAction == PendingAction.create) {
          ...
          DebugPerfLogger.end(
            ...
            eventWatch,
            'pushEvent.create',
            data: {'id': normalizedEvent.id, 'remoteId': created.gEventId},
          );
          ...
          if (normalizedEvent.gEventId == null) {
            ...
            DebugPerfLogger.end(
              ...
              'pushEvent.update',
              data: {
                'id': eventForRemote.id,
                'remoteId': eventForRemote.gEventId,
              },
            );
          }
          ...
          DebugPerfLogger.end(
            ...
            data: {
              ...
              'remoteId': normalizedEvent.gEventId,
            },
          );
        }
      } catch (e) {
        if (_isMissingRemoteError(e)) {
          try {
            ...
            if (resolved) {
              debugPrint(
                'Recovered missing remote event during push: '
                'id=${normalizedEvent.id} action=${normalizedEvent.pendingAction.name}',
              );
              continue;
            }
          } catch (recoveryError) {
            debugPrint(
              'Missing-remote recovery failed for ${normalizedEvent.id}: $recoveryError',
            );
          }
        }
        DebugPerfLogger.error(
          ...
          data: {
            'id': normalizedEvent.id,
            'pendingAction': normalizedEvent.pendingAction.name,
          },
        );
        debugPrint(
          'Local push failed for event ${normalizedEvent.id} '
          '(action=${normalizedEvent.pendingAction.name}, '
          'calendarId=${normalizedEvent.calendarId}, '
          'gEventId=${normalizedEvent.gEventId}): $e',
        );
  ...
  Future<bool> _resolveMissingRemoteDuringPush(CalendarEvent event) async {
    if (event.pendingAction == PendingAction.delete) {
      await _localStore.deleteEventById(event.id);
      return true;
    }

    if (event.pendingAction != PendingAction.update) {
      return false;
    }
    ...
    final latestLocal = await _localStore.getById(event.id);
    if (_hasNewerLocalMutation(latestLocal, event)) {
      final rebased = _rebaseLocalEventOnRemoteAck(
        latest: latestLocal!,
        pushedSnapshot: event,
        remoteGEventId: recreated.gEventId,
        remoteUpdatedAt: remoteUpdatedAt,
      ).copyWith(id: remoteLocalId);
      await _upsertEventWithIdentityTransition(oldId: event.id, event: rebased);
    } else {
      await _upsertEventWithIdentityTransition(oldId: event.id, event: updated);
    }
    if (updated.gEventId != null) {
      await _localStore.removeDuplicateGoogleEventCopies(
        gEventId: updated.gEventId!,
